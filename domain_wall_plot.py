"""
Description:
    For making plot showing comparison of box plots (cheese slice voxel image plots)
"""
...
def set_box_plots_3d(file_dir_1, file_dir_2, file_dir_3, N = 5, vmin = 0, vmax = 255, diagF = True):
    files_in_dir_1 = os.listdir(file_dir_1)
    files_in_dir_1 = [s for s in files_in_dir_1 if "pars.npy" not in s]
    files_in_dir_1 = sorted(files_in_dir_1, key = lambda x: int(os.path.splitext(x)[0]))
    
    files_in_dir_2 = os.listdir(file_dir_2)
    files_in_dir_2 = [s for s in files_in_dir_2 if 'pars.npy' not in s]
    files_in_dir_2 = sorted(files_in_dir_2, key = lambda x: int(os.path.splitext(x)[0]))
    
    files_in_dir_3 = os.listdir(file_dir_3)
    files_in_dir_3 = [s for s in files_in_dir_3 if 'pars.npy' not in s]
    files_in_dir_3 = sorted(files_in_dir_3, key = lambda x: int(os.path.splitext(x)[0]))
    
    if len(files_in_dir_1)%N == 0:
        step_1 = int(len(files_in_dir_1)/N)
    else:
        step_1 = int(len(files_in_dir_1)/N) + 1
        
    if len(files_in_dir_2)%N == 0:
        step_2 = int(len(files_in_dir_2)/N)
    else:
        step_2 = int(len(files_in_dir_2)/N) + 1
        
    if len(files_in_dir_3)%N == 0:
        step_3 = int(len(files_in_dir_3)/N)
    else:
        step_3 = int(len(files_in_dir_3)/N) + 1
        
    print('step_1, step_2, step_3', step_1, step_2, step_3)
    # Create a figure with 3D ax
    fig = plt.figure(figsize=(12, 4))
    
    for i in range(N):
        ax = fig.add_subplot(3, N, i + 1, projection='3d')
        ax.set_title(f'{files_in_dir_1[i*step_1]}')
        
        data = tifffile.imread(f'{file_dir_1}//{files_in_dir_1[i*step_1]}')
        if diagF:
    ...
    for i in range(N):
        ...
        ax.set_title(f'{files_in_dir_2[i*step_2]}')
        
        data = tifffile.imread(f'{file_dir_2}//{files_in_dir_2[i*step_2]}')
        data = 127 - 1*(data - 127.5)
        if diagF:
            diag_box_plot_3d(data, ax, vmin, vmax)
        else:
            box_plot_3d(data, ax, vmin, vmax)
    ...
    for i in range(N):
        ...
        ax.set_title(f'{files_in_dir_3[i*step_3]}')
        
        data = tifffile.imread(f'{file_dir_3}//{files_in_dir_3[i*step_3]}')
        if diagF:
            diag_box_plot_3d(data, ax, vmin, vmax)
        else:
            box_plot_3d(data, ax, vmin, vmax)     
...
if __name__ == '__main__':
    # ~ set_box_plots_3d(r'results\default\time_sequence',  N = 5, diagF = True)

    set_box_plots_3d(r'results\domain_walls\pristine\time_sequence_2',
                     r'results\domain_walls\random_field\time_sequence_2_c',
                     r'results\domain_walls\random_mass\time_sequence_2',
                     N = 5, diagF = True)
