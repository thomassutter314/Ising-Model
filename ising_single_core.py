...
class Model():
    def __init__(self, temperature = 2,\
        ...
        # ~ self.L = C_size*15 # Size of the lattice
        # ~ if self.L%2 != 0:
            # ~ self.L += 1
        ...
        self.C.append([int(self.L/2), int(self.L/2)])
        
        self.C = np.array(self.C).transpose()
        print(self.C)
        print(self.C.shape)
                
        self.C_type = np.ones(C_count**2)
        ...
        self.m_mask = self.mass/np.max(self.mass)
    ...
    def measure_g2(self):
        ...
        tifffile.imsave(r'C:\Users\thoma\Documents\GitHub\Ising-Model\results\default\diffraction\avg_diffraction_image.tiff', np.mean(Iframes, axis = 0))
        ...
        for i in range(F.shape[0]):
            ax[0].plot(lag_steps[1:], F[i], label = f'q_radius = {i*q_radius}')
...
def make_plots(save_name = '0', fname = r"C:\Users\thoma\Documents\GitHub\Ising-Model\results\default\g2.csv", 
    ...
    np.savetxt(f'{save_loc}\\F2_{save_name}.csv', F, delimiter = ',')
    ...
    for i in range(F.shape[0]):
        ...
        ax.scatter(data[0,step_in:], F_new, color = cmap(i/F.shape[0]), s = 5)
        p0 = [tau, 1, 1]
        try:
            # ~ popt, pcov = opt.curve_fit(sc_exponential, lag_steps_new[:fit_in], F_new[:fit_in], p0 = [tau, beta])
            popt, pcov = opt.curve_fit(exponential_decay, lag_steps_new[fit_out:fit_in], F_new[fit_out:fit_in], p0 = p0)
            taus.append(popt[0])
            ...
            plt.plot(tt, yy, label = f'tau = {round(popt[0],2)}' + r'$\pm$' + f'{round(np.sqrt(pcov[0,0]),2)}'  + \
                                     f', beta = {round(popt[1],2)}' + r'$\pm$' + f'{round(np.sqrt(pcov[1,1]),2)}', color = cmap(i/F.shape[0]))
            ...
            plt.plot(tt, yy, label = f'tau = {round(p0[0],2)}', color = cmap(i/F.shape[0]))
...
def update(config, field, mass, temp, N):
    ...
    for n in range(N):
        ...
        if I[n] + 1 < config.shape[0] and J[n] + 1 < config.shape[1]:
            nn = config[(I[n] + 1), J[n]] + config[(I[n] - 1), J[n]] + config[I[n], (J[n] + 1)] + config[I[n], (J[n] - 1)]
        else:
            nn = 0
...
def fit_func_beta_1(t, tau, beta):
    beta = 1
    return np.exp(-(np.abs(t)/np.abs(tau))**np.abs(beta))
...
def analyze_telegraph(signal):
    ...
    fit_interval = 56005
    tt = np.linspace(min(t_corr),max(t_corr),50000)
    p0 = [500, 1]
    yy_guess = fit_func(tt, *p0)
    popt, pcov = sci.optimize.curve_fit(fit_func, t_corr, corr, p0 = p0)
    popt_beta_1, pcov_beta_1 = sci.optimize.curve_fit(fit_func_beta_1, t_corr, corr, p0 = p0)
    print('telegraph')
    print('tau, beta', popt)
    print('tau, beta = 1', popt_beta_1)
...
model = Model(temperature = temp,
              step_number = 2000001, initial_step_number = 10000, exposure_time = 50,
              C_field_strength = 0.000, C_mass_strength = 0.4, C_size = 7,
              real_space_images_to_save = 500, gen_new_fields = True)


make_plots(save_name = temp, fit_in = 17, fit_out = 0)
